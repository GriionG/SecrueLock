...
<script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
	<script>
        ...
        window.location = "Inventario.php?id_eliminar=" + id;
 
      }

      function modificar(id){
         window.location = "Inventario.php?idmodificar=" + id;
      }
...
if (isset($_POST['btninsertar'])){
    ...
    @$result = $obj->Ejecutar_Instruccion("INSERT INTO medicamento(medicamento, fecha_cad,stock) values('$medicamento','$fecha_cad','$stock')");
    echo '<script>window.location = "Inventario.php"; </script>';
    }
    else if (isset($_GET['id_eliminar']))
        {
            $id = $_GET['id_eliminar'];
             $obj->Ejecutar_Instruccion("delete from medicamento where id_medicamento = '$id'");
        }
        if (isset($_POST['btnbuscar'])){
@$buscar = $_POST['txtbuscar'];
$result = @$obj->Ejecutar_Instruccion("select * from medicamento WHERE medicamento LIKE '%$buscar%' or Estado LIKE '%$buscar%' ");
}
else{
$result = $obj->Ejecutar_Instruccion("select * from medicamento");
}
?>
   <!--::header part start::-->
    <header class="">
...
?>
	...
	<div class="whole-wrap">
		<div class="container box_1170"> 
            ...
            <div class="row">
					<div class="col-lg-4">
						<form action="Inventario.php" id="frminsertar" name="frminsertar" method="post"  enctype="multipart/form-data">
						<input type="hidden" name="txtid_medicamento" id="txtid_medicamento" placeholder="ID" class="single-input">
							<div class="mt-10">
								<p>Medicamento </p>
								<input type="text" name="txtmedicamento" id="txtmedicamento" placeholder="Medicamento"
									onfocus="this.placeholder = ''" onblur="this.placeholder = 'Medicamento'" required
									class="form-control">
							</div>
							<div class="mt-10">
							<p>Fecha Caducidad </p>
								<input type="date" name="txtfecha_cad" id="txtfecha_cad" placeholder="Fecha Caducidad"
									onfocus="this.placeholder = ''" onblur="this.placeholder = 'Fecha Caducidad'" required
									class="form-control">
							</div>
                            <div class="mt-10">
							<p>Stock</p>
								<input type="text" name="txtstock" id="txtstock" placeholder="Stock"
									onfocus="this.placeholder = ''" onblur="this.placeholder = 'Stock'" required
									class="form-control">
							</div>
                            <br>
	...
	?>
			<td style="text-align:center">
                        <a type="button" class="btn px-3 btn_3" id="btneliminar" name="btneliminar" onclick="javascript:eliminar('<?php echo $renglon[0];?>');"><i style="color:white" class="ti-trash"></i></a>
                        <a type="button" class="btn px-3 btn_3 editbtn" data-toggle="modal" value="Modificar" data-target="#editar"><i style="color:white" class= "ti-pencil-alt"></i></a></td>
		...
		<!-- Modal Editar -->
                  <div class="modal fade" id="editar" tabindex="-1" role="dialog" >
                    <div class="modal-dialog" role="document">
                     ...
                     <!--formulario-->
                     <form action="editar-inventario.php" method="POST" enctype="multipart/form-data">
                                    <input class="input" type="hidden" name="txtid_medicamento" id="id_medicamento" placeholder="IDproducto">
                                     <div class="form-group">
                                        <input type="text" class="form-control" name="txtmedicamento" id="medicamento" placeholder="Medicamento">
                                     </div>
                                     <div class="form-group">
                                        <input type="text" class="form-control" name="txtfecha_cad" id="fecha_cad" placeholder="Fecha caducidad">
                                     </div>
                                     <div class="form-group">
                                        <input type="text" class="form-control" name="txtstock" id="stock" placeholder="Existencias">
							        </div>
               ...
               $('.editbtn').on('click',function () {
                  ...
                  $('#id_medicamento').val(datos[0]);
                  $('#medicamento').val(datos[1]);
                  $('#fecha_cad').val(datos[2]);
                  $('#stock').val(datos[3]);
               });   
               </script>
